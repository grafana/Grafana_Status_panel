...
import { ColorPicker, HorizontalGroup, Label } from '@grafana/ui';
  ...
  return (
    <HorizontalGroup spacing="lg">
      <div>
        ...
        <ColorPicker color={value?.crit ||config.theme2.visualization.getColorByName("red")} onChange={buildHandler('crit')} />
        {/* {colorPicker({ value: value.crit, onChange: buildHandler('crit') })} */}
      </div>
      <div>
        <Label>Warning</Label>
        {/* {colorPicker({ value: value.warn, onChange: buildHandler('warn') })} */}
        <ColorPicker color={value?.warn || config.theme2.visualization.getColorByName("orange")} onChange={buildHandler('warn')} />
      </div>
      <div>
        <Label>OK</Label>
        {/* {colorPicker({ value: value.ok, onChange: buildHandler('ok') })} */}
        <ColorPicker color={value?.ok || config.theme2.visualization.getColorByName("green")} onChange={buildHandler('ok')} />
      </div>
      <div>
        <Label>Disabled</Label>
        <ColorPicker color={value?.disable || config.theme2.visualization.getColorByName("gray")} onChange={buildHandler('disable')} />
