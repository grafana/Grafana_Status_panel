...
- **Fork**: `SckyzO/Grafana_Status_panel` (from `grafana/Grafana_Status_panel`, branch `main`, v2.0.4)
- **Plugin id**: `vonage-status-panel` (kept — drop-in updates for existing dashboards)
...
- **v3.0 — Evolution**: multi-metric card model, threshold steps, multicard, per-metric mappings — behind an **automatic v2→v3 migration**, plugin id preserved.
...
| ----------------------------- | ----------------------------------------------------------------- | ---------- |
| **Value-handler registry**    | replace hardcoded `switch` → extensible without touching the core | —          |
...
| **Per-metric option box**     | auto-generated from `context.data`, keyed by `refId` (see §6)     | —          |
...
- **Directional mappings**: mapping is arbitrary per metric — IPMI `0→ok … 2→crit` (increasing) _and_ bonding `2→ok … 0→crit` (decreasing). No implicit direction.
